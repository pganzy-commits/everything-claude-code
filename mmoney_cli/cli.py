...
# Shared auth: also check the MCP server's keyring entry
_MCP_KEYRING_SERVICE = "com.mcp.monarch-mcp-server"
_MCP_KEYRING_USERNAME = "monarch-token"


def save_token_to_keychain(token: str) -> bool:
    """Save auth token to system keychain (both CLI and MCP entries).
    ...
    try:
        ...
        # Also save to MCP keyring so both tools share the session
        keyring.set_password(_MCP_KEYRING_SERVICE, _MCP_KEYRING_USERNAME, token)
        return True
    except Exception:
        return False
...
def load_token_from_keychain() -> str | None:
    ...
    Checks CLI keyring first, then falls back to MCP server keyring
    so auth from either tool works for both.
    ...
    # Fall back to MCP server's keyring entry
    try:
        token = keyring.get_password(_MCP_KEYRING_SERVICE, _MCP_KEYRING_USERNAME)
        if token:
            return token
    except Exception:
        pass
...
def delete_token_from_keychain() -> bool:
    """Delete auth token from system keychain (both CLI and MCP entries).
    ...
    try:
        ...
        pass
    try:
        keyring.delete_password(_MCP_KEYRING_SERVICE, _MCP_KEYRING_USERNAME)
        deleted = True
    except Exception:
        pass
    return deleted
...
def get_client() -> MonarchMoney:
    ...
    Tries keychain first (CLI then MCP), then falls back to pickle file in ~/.mmoney/.
    """
    mm = MonarchMoney(session_file=str(_SESSION_FILE))

    # Try keychain first (checks both CLI and MCP keyring entries)
