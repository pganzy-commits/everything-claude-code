...
class TestKeychainStorage:
    ...
    def test_save_token_to_keychain_success(self):
        """Test saving token to keychain (saves to both CLI and MCP entries)."""
        ...
        with patch("mmoney_cli.cli.keyring") as mock_keyring:
            ...
            mock_keyring.set_password.assert_any_call(
                "com.mcp.monarch-mcp-server", "monarch-token", "test_token"
            )
    ...
    def test_delete_token_from_keychain_success(self):
        """Test deleting token from keychain (deletes both CLI and MCP entries)."""
        ...
        with patch("mmoney_cli.cli.keyring") as mock_keyring:
            ...
            mock_keyring.delete_password.assert_any_call("mmoney-cli", "monarch-token")
            mock_keyring.delete_password.assert_any_call(
                "com.mcp.monarch-mcp-server", "monarch-token"
            )
